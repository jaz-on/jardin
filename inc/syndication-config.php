...
function jardin_mastodon_status( $text, $post ) {
	$post = get_post( $post );
	$updates_pt = ( defined( 'JARDIN_UPDATES_POST_TYPE' ) && post_type_exists( JARDIN_UPDATES_POST_TYPE ) ) ? JARDIN_UPDATES_POST_TYPE : '';
	if ( ! $post || ! in_array( (string) $post->post_type, array_filter( array( 'post', $updates_pt ) ), true ) ) {
		return $text;
	}
	if ( '' === $updates_pt || $updates_pt !== (string) $post->post_type ) {
		return $text;
	}
...
function jardin_webmention_post_types( array $types ): array {
	$our = array( 'post', 'project', 'event', 'favorite', 'blogroll', 'beer_checkin', 'listen', 'iwcpt_note', 'iwcpt_like' );
	if ( defined( 'JARDIN_UPDATES_POST_TYPE' ) && post_type_exists( JARDIN_UPDATES_POST_TYPE ) ) {
		$our[] = JARDIN_UPDATES_POST_TYPE;
	}
