...
function jardin_mastodon_status( $text, $post ) {
	$post = get_post( $post );
	if ( ! $post || ! in_array( (string) $post->post_type, array( 'post', JARDIN_UPDATES_POST_TYPE ), true ) ) {
		return $text;
	}
	if ( JARDIN_UPDATES_POST_TYPE !== (string) $post->post_type ) {
		return $text;
	}
...
function jardin_webmention_post_types( array $types ): array {
	$our = array( 'post', 'project', 'event', 'favorite', 'blogroll', 'beer_checkin', 'listen', 'iwcpt_note', 'iwcpt_like', JARDIN_UPDATES_POST_TYPE );
